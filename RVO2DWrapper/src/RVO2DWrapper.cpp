...
#include "../include/RVO2DWrapper.h"
...
RVOSimulator::RVOSimulator(float timeStep, float neighborDist, int maxNeighbors, float timeHorizon,float obsHorizon ,float radius, float maxSpeed, RvoVector2 velocity)
{
...
int RVOSimulator::addAgent(RvoVector2 position)
{
...
int RVOSimulator::addAgent(RvoVector2 position, float neighborDist, int maxNeighbors, float timeHorizon,float timeHorizonObst, float radius, float maxSpeed, RvoVector2 velocity )
{
	RVO::Vector2 pos = RVO::Vector2(position.X,position.Y);	
...
void RVOSimulator::getAgentPositions(array<int>^ agentNo, array<RvoVector2>^ positions) 
{
	positions = gcnew array<RvoVector2>(agentNo->Length);
	for(int i =0 ; i < agentNo->Length; i++)
	{
		RVO::Vector2 v = _sim->getAgentPosition(agentNo[i]);
		positions[i] = RvoVector2( v.x(),v.y());		
	}
}

void RVOSimulator::getAgentPrefVelocities(array<int>^ agentNo,array<RvoVector2>^ prefVelocities) 
{
	prefVelocities = gcnew array<RvoVector2>(agentNo->Length);
	for(int i =0 ; i < agentNo->Length; i++)
	{
		RVO::Vector2 v = _sim->getAgentPrefVelocity(agentNo[i]);
		prefVelocities[i] = RvoVector2( v.x(),v.y());		
	}
}

RvoVector2  RVOSimulator::getAgentPosition(int agentNo) 
{
	 RVO::Vector2 v = _sim->getAgentPosition(agentNo);
	 RvoVector2 vec = RvoVector2( v.x(),v.y());
	 return vec;
}

RvoVector2  RVOSimulator::getAgentPrefVelocity(int agentNo) 
{
...
RvoVector2 RVOSimulator::getAgentVelocity(int agentNo) 
{
...
void RVOSimulator::setAgentDefaults(float neighborDist, int maxNeighbors, float timeHorizon,float obsHorizon, float radius, float maxSpeed, RvoVector2 velocity)
{
	RVO::Vector2 vel= RVO::Vector2(velocity.X,velocity.Y);	
...
void RVOSimulator::setAgentPosition(int agentNo,  RvoVector2 position)
{
	RVO::Vector2 pos= RVO::Vector2(position.X,position.Y);	
	_sim->setAgentPosition(agentNo,pos);
}
void RVOSimulator::setAgentPrefVelocity(int agentNo,  RvoVector2 velocity)
{
	RVO::Vector2 vel= RVO::Vector2(velocity.X,velocity.Y);	
	_sim->setAgentPrefVelocity(agentNo,vel);
}

void RVOSimulator::setAgentPrefVelocity(array<int>^ agentNo,  array<RvoVector2>^ prefVelocity)
{
	for(int i =0 ; i < agentNo->Length; i++)
	{
...
void RVOSimulator::setAgentVelocity(int agentNo,  RvoVector2 velocity)
{
	RVO::Vector2 vel= RVO::Vector2(velocity.X,velocity.Y);	
...
void RVOSimulator::setAgentVelocity(array<int>^ agentNo,  array<RvoVector2>^ velocity)
{
	for(int i =0 ; i < agentNo->Length; i++)
	{
...
int RVOSimulator::addObstacle(System::Collections::Generic::List<RvoVector2>^ vertices)
{
...
int RVOSimulator::addObstacle(array<RvoVector2>^ vertices)
{
	std::vector<RVO::Vector2> vert;	
...
bool RVOSimulator::queryVisibility(RvoVector2 point1, RvoVector2 point2, float radius) 
{
